    ...
    <section className="reveal-section relative max-w-7xl mx-auto py-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-12">
        <div className="flex flex-col justify-center text-left">
          {subtitle ? (
            <p className="text-sm text-gray-500 mb-3 tracking-wider reveal-el">{subtitle}</p>
          ) : null}
          <h1 className="text-5xl md:text-6xl font-primary leading-tight reveal-el">{title}</h1>
          {description ? (
            <p className="mt-4 text-gray-600 max-w-sm leading-relaxed reveal-el">{description}</p>
          ) : null}
        </div>
