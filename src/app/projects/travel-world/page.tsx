...
const hoverTiles = [
  { title: "Luxury Beach Getaways", subtitle: "Warm sands, calm seas", img: "/assets/travelworld_laptop_4k_transparent_crisp 1.png" },
  { title: "Mountain Adventures", subtitle: "Fresh air, scenic trails", img: "/assets/Travel World Second Section .png" },
  { title: "Cruise Experiences", subtitle: "Sail in style", img: "/assets/Travel World Second Section .png" },
];
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context((self) => {
      ...
      (gsap.utils.toArray<HTMLElement>(select("section.reveal-section")) || []).forEach((sec) => {
        ...
        gsap.fromTo(sec, { y: 28, autoAlpha: 0 }, {
          y: 0, autoAlpha: 1, duration: 0.7, ease: "power2.out", immediateRender: false,
          scrollTrigger: { trigger: sec, start: "top 85%", end: "bottom 25%", toggleActions: "play reverse play reverse" },
        });

        if (children.length) {
          gsap.fromTo(children, { y: 20, autoAlpha: 0 }, {
            y: 0, autoAlpha: 1, duration: 0.6, ease: "power2.out", immediateRender: false, stagger: 0.08,
            scrollTrigger: { trigger: sec, start: "top 85%", end: "bottom 25%", toggleActions: "play reverse play reverse" },
          });
        }
      });
      ...
      ScrollTrigger.batch(select(".pop-on-scroll"), {
        start: "top 85%",
        onEnter: (batch: Element[]) => gsap.to(batch, { scale: 1, autoAlpha: 1, duration: 0.5, ease: "back.out(1.6)", stagger: 0.06 }),
        onLeave: (batch: Element[]) => gsap.to(batch, { scale: 0.9, autoAlpha: 0, duration: 0.3, ease: "power1.inOut", stagger: 0.05 }),
        onEnterBack: (batch: Element[]) => gsap.to(batch, { scale: 1, autoAlpha: 1, duration: 0.45, ease: "back.out(1.4)", stagger: 0.06 }),
        onLeaveBack: (batch: Element[]) => gsap.to(batch, { scale: 0.9, autoAlpha: 0, duration: 0.25, ease: "power1.inOut", stagger: 0.05 }),
      });
    }, root);

    return () => {
      try { ctx.revert(); } finally { try { delete (root as any).dataset.gsapInit; } catch {} }
    };
  }, []);
    ...
    <div ref={rootRef} className="relative overflow-x-hidden space-y-16 md:space-y-24">
      ...
      <section className="reveal-section group py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <h2 className="text-[var(--text)] font-primary text-3xl md:text-4xl mb-8">Explore by hovering</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {hoverTiles.map((t) => (
              <div key={t.title} className="group relative overflow-hidden rounded-2xl border border-white/10 shadow-md bg-[var(--background)]">
                <div className="relative w-full aspect-[16/10]">
                  <Image src={t.img} alt={t.title} fill sizes="(max-width: 1024px) 100vw, 33vw" className="object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
                </div>
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 flex items-end justify-between opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all">
                  ...
                  <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white/90 text-[var(--accent)] font-bold shadow">↗</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-9 sm:px-6 lg:px-8 min-h-[60vh] flex items-center relative z-10">
          <div className="mx-auto grid w-full max-w-6xl grid-cols-1 lg:grid-cols-2 gap-10 items-center justify-items-center">
            <div className="col-span-1 text-center">
              ...
              <p className="reveal-el text-[var(--secondary-text)] text-lg md:text-xl leading-relaxed max-w-md mx-auto text-center">A simple, friendly travel website that makes exploring destinations feel fun and effortless.</p>
            </div>
            <div className="relative mx-auto w-full max-w-[2100px] px-8 md:px-12 lg:px-16 overflow-hidden">
              <div className="reveal-el scale-[1.15] md:scale-[1.2] transition-transform duration-500 ease-out group-hover:scale-[1.23]">
                <Image src="/assets/travelworld_laptop_4k_transparent_crisp 1.png" alt="Travel World laptop mockup" width={2000} height={1125} priority className="w-full h-auto object-contain" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
            <div className="pop-on-scroll rounded-2xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-[var(--Secondary-Background)]/70 shadow-sm p-6">
              <div className="flex items-center gap-2 mb-3"><CheckCircle2 size={18} /><h4 className="font-primary text-lg text-[var(--text)]">What I did</h4></div>
            ...
            <div className="pop-on-scroll rounded-2xl border border-black/10 dark:border-white/10 bg-white/80 dark:bg-[var(--Secondary-Background)]/70 shadow-sm p-6">
              <div className="flex items-center gap-2 mb-3"><Target size={18} /><h4 className="font-primary text-lg text-[var(--text)]">Focus</h4></div>
              <ul className="space-y-2 text-sm text-[var(--secondary-text)]">
      ...
      <section className="reveal-section relative w-full py-16 md:py-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative flex items-center justify-center gap-8" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
            <button onClick={handlePrevTheme} aria-label="Previous theme" className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 p-3 rounded-full bg-[var(--Secondary-Background)]/70 hover:bg-[var(--Secondary-Background)] text-[var(--text)] shadow-lg border border-white/10 backdrop-blur">
              <ChevronLeft size={28} />
            </button>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-5xl">
              <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-xl ring-1 ring-black/5 border border-white/10 bg-[var(--background)]">
                <Image key={`${currentTheme.id}-a`} src={currentImage} alt={`${currentTheme.title} - Image ${currentImageIndex + 1}`} fill className="object-cover transition-opacity duration-500" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
              <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-xl ring-1 ring-black/5 border border-white/10 bg-[var(--background)]">
                <Image key={`${currentTheme.id}-b`} src={currentTheme.images[(currentImageIndex + 1) % currentTheme.images.length]} alt={`${currentTheme.title} - Image ${((currentImageIndex + 1) % currentTheme.images.length) + 1}`} fill className="object-cover transition-opacity duration-500" sizes="(max-width: 768px) 100vw, 50vw" />
              </div>
            </div>
            <button onClick={handleNextTheme} aria-label="Next theme" className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 p-3 rounded-full bg-[var(--Secondary-Background)]/70 hover:bg-[var(--Secondary-Background)] text-[var(--text)] shadow-lg border border-white/10 backdrop-blur">
          ...
          <div className="flex items-center justify-center gap-2 mt-3">
            {themes.map((t, idx) => (
              <button key={t.id} aria-label={`Go to ${t.title}`} onClick={() => { setCurrentThemeIndex(idx); setCurrentImageIndex(0); }} className={`h-2.5 w-2.5 rounded-full transition-all ${idx === currentThemeIndex ? "bg-[var(--accent)] w-6" : "bg-[var(--secondary-text)]/30"}`} />
            ))}
          </div>
      ...
      <section className="reveal-section relative w-full py-16 md:py-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h2 className="reveal-el text-[var(--text)] font-primary text-4xl md:text-5xl mb-12">WHERE DO YOU WANT TO OPEN TRAVEL WORLD?</h2>
          <div className="max-w-3xl md:max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 items-stretch">
            <a href="https://travel-website-complete-w0th.onrender.com/index.html" target="_blank" rel="noopener noreferrer" className="pop-on-scroll ticket-button group relative rounded-xl bg-[var(--accent)] text-white shadow-lg ring-1 ring-white/10 transition-all duration-200 hover:-translate-y-[2px] w-full" aria-label="Open live website">
            ...
            <a href="https://www.figma.com/design/rzN9N9qsHKqxYUwQpsEwfn/Untitled?node-id=0-1&p=f&t=aGmk5l4FXR8dXmtn-0" target="_blank" rel="noopener noreferrer" className="pop-on-scroll ticket-button group relative rounded-xl bg-[var(--accent)] text-white shadow-lg ring-1 ring-white/10 transition-all duration-200 hover:-translate-y-[2px] w-full" aria-label="Open Figma design file">
              <div className="pointer-events-none absolute inset-0 opacity-20 bg-gradient-to-br from-white/15 via-white/0 to-black/10" />
              <div className="relative flex items-center justify-between gap-4 px-5 md:px-6 py-4">
                <div className="flex items-center gap-4">
                  <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-white text-[var(--accent)] shadow-sm">🎨</span>
